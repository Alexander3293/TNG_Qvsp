...
single_segd_files::single_segd_files( QObject *parent) :
    QObject(parent),
...
single_segd_files::~single_segd_files()
{
...
void single_segd_files::setTime(const QTime &time)
{
...
void single_segd_files::setData(const QDate &date)
{
    year_   = (quint8) hex_from_dec(date.year()%100);
    day_    = (quint16)hex_from_dec(date.dayOfYear());
}

void single_segd_files::write_spacer()
{
...
unsigned char* single_segd_files::convertData(double value)
{
...
unsigned char *single_segd_files::convertData2(double value)
{
    unsigned char* result = new unsigned char[4];
    for(int i = 0; i < 4; i++)
        result[i] = 0;
...
void single_segd_files::write_general_header()
{
...
void single_segd_files::write_general_header_blk2()
{
    if (!file_->open(QIODevice::Append))
    {
        qDebug() << file_->errorString();
    ...
    generalHeader <<  (quint8) 3;   //major revision number and minor revision number
    ...
    generalHeader <<  (quint16) hex_from_dec(0);    //not used
    generalHeader <<  (quint16) hex_from_dec(0);    //not used
    /* Extended Record  3 bytes general_header_blk1 bytes 26 & 27 sets 0xFFF, 17-20 bytes*/
    generalHeader <<  (quint8) 0x00;
    generalHeader <<  (quint8) 0x1e;
    generalHeader <<  (quint8) 0x84;
    generalHeader <<  (quint8) 0x80;
    ...
    generalHeader <<  (quint16) hex_from_dec(0);
    generalHeader <<  (quint8) hex_from_dec(0);
    /* Extended Header Blks 28-30 */
    generalHeader <<  (quint16) hex_from_dec(0);
    generalHeader <<  (quint8) hex_from_dec(0);

    generalHeader <<  (quint8) hex_from_dec(0); //Undefinded
...
void single_segd_files::write_general_header_blk3()
{
    if (!file_->open(QIODevice::Append))
    {
        qDebug() << file_->errorString();
    ...
    /* Header Size (25-28 bytes) */
    generalHeader <<  (quint16) hex_from_dec(0);
    generalHeader <<  (quint16) hex_from_dec(0);

    /* Extnd Rec Mode (29 bytes) */
    generalHeader <<  (quint8) hex_from_dec(1); // Tyt Length data

    /* Rel Time Mode (30 bytes) */
    generalHeader <<  (quint8) hex_from_dec(0);

    generalHeader <<  (quint8) hex_from_dec(0); //Undefinded
    /*Header Block Type  */
    generalHeader <<  (quint8) hex_from_dec(3);
...
void single_segd_files::write_header()
{
    if (!file_->open(QIODevice::Append))
    {
        qDebug() << file_->errorString();
    ...
    header << (quint16) hex_from_dec (channelSetNumber_);           // [2-3]
    header << (quint8) (channelType_ << 4); // chtype[4 bits] + 0[4 bits]   //4
    header << (quint32)  ((int)channelSetStartTime_*(1000/2));      // [5-8]
    header << (quint32)  ((int)channelSetEndTime_*(1000/2));        // [9-12]
    header << (quint32) numOfSamples_;                              // [13-16] number of samples
    header << (quint32) hex_from_dec (descaleMultiplier_);          // [17-20]
    header << (quint8) hex_from_dec (0);                            // [21-23]
    header << (quint16) hex_from_dec (numberOfChannels_);           // [21-23]
    ...
    header << (quint8) (1);
    /* Gain 29*/
    header << (quint8) (channelGainControlMethod_);
    /* Vertical Stack 30*/
    header << (quint8) (1);
    ...
    header << (quint32) hex_from_dec (aliasFilterFrequency_);
    /* Low-cut filter Setting 37-40*/
    header << (quint32) hex_from_dec (lowCutFilter_);
    /* Alias filter slope */
    header << (quint32) hex_from_dec (aliasFilterSlope_);
    /* Low-cut filter slope 45-48*/
    header << (quint32) hex_from_dec (lowCutFilterSlope_);
    /* Notch freq setting 49-52*/
    header << (quint32) hex_from_dec (0);
    /* Second notch filter 53-56*/
    header << (quint32) hex_from_dec (0);
    /* Third notch filter 57-60*/
    header << (quint32) hex_from_dec (0);
    /* Filter phase 61*/
    header << (quint8) hex_from_dec (0);
    /* Physical Unit 62*/
    header << (quint8) hex_from_dec (0);
    /* Undefinded 63*/
    header << (quint8) hex_from_dec (0);
...
void single_segd_files::write_extended_header()
{
    if (!file_->open(QIODevice::Append))
    {
...
int single_segd_files::counterByte() const
{
    return counterByte_;
}

void single_segd_files::setCounterByte(int counterByte)
{
    counterByte_ = counterByte;
}

uint16_t single_segd_files::bit_to_byte(QBitArray* bitArray)
{
...
void single_segd_files::bit_to_byte(QBitArray* bitArray, uint8_t returnValue)
{
    QByteArray bytes;
...
void single_segd_files::byte_to_bit(QBitArray *returnValue, int inValue)
{
    int ch = 2;
    for(int i = 0; i < ch; ++i)
    {
...
int single_segd_files::hex_from_dec(int dec)
{
    int hex = 0;
    int tmp = dec;
    int i = 1;
...
int single_segd_files::dec_from_hex(int hex)
{
    int dec = 0;
    int tmp = hex;
    int i = 1;
...
quint8 single_segd_files::channelSets() const
{
    return channelSets_;
}

void single_segd_files::setChannelSets(const quint8 &channelSets)
{
    channelSets_ = channelSets;
}

void single_segd_files::openFile(QString fileReadName)
{
...
void single_segd_files::read_and_write()
{
    QDataStream streamData;
    streamData.setDevice(fileRead_);
    streamData.setVersion(QDataStream::Qt_5_9);
...
uint32_t single_segd_files::getNumOfSamples()
{
    return numOfSamples_;
}

void single_segd_files::setAliasFilterSlope(const quint16 &aliasFilterSlope)
{
    aliasFilterSlope_ = aliasFilterSlope;
}

void single_segd_files::setDataLength(const quint32 &numOfSamples)
{
    numOfSamples_ = numOfSamples;
}

void single_segd_files::setChannelGainControlMethod(const quint8 &channelGainControlMethod)
{
    channelGainControlMethod_ = channelGainControlMethod;
}

void single_segd_files::setChannelSetEndTime(const quint16 &channelSetEndTime)
{
    channelSetEndTime_ = channelSetEndTime;
}

void single_segd_files::setChannelSetStartTime(const quint16 &channelSetStartTime)
{
    channelSetStartTime_ = channelSetStartTime;
}

void single_segd_files::setScanTypeNumber(const quint8 &scanTypeNumber)
{
    scanTypeNumber_ = scanTypeNumber;
}

quint16 single_segd_files::fileNumber() const
{
    return fileNumber_;
}

void single_segd_files::setFileNumber(const quint16 &fileNumber)
{
    fileNumber_ = fileNumber;
}

void single_segd_files::setFileName(QString fileName)
{
    fileName_ = fileName;
    file_->setFileName(fileName_);
}
QString single_segd_files::getFileName()
{
    return fileName_;
}
void single_segd_files::setNumGenBlks(uint8_t num)
{
    numGenBlks_ = num;
}

void single_segd_files::addRecordLength(quint16 &inputVal, quint16 length)
{
    inputVal = length << 4;
}

void single_segd_files::write_data(double x1, unsigned int channelSet, quint16 channel)
{
    if (!file_->open(QIODevice::Append))
    {
        qDebug() << file_->errorString();
...
/* demux trace header */
void single_segd_files::write_data_header( unsigned int channelSet, quint16 channel)
{
    QDataStream stream;
    stream.setDevice(file_);
    stream.setVersion(QDataStream::Qt_5_9);
...
void single_segd_files::append_data(QVector<float> data)
{
    QDataStream stream;
    stream.setDevice(file_);
    stream.setVersion(QDataStream::Qt_5_9);
...
void single_segd_files::append_data(QVector<bool> data)
{
    QDataStream stream;
    stream.setDevice(file_);
    stream.setVersion(QDataStream::Qt_5_9);
...
void single_segd_files::append_data2(QVector<float> data)
{
    QDataStream stream;
    stream.setDevice(file_);
    stream.setVersion(QDataStream::Qt_5_9);
...
void single_segd_files::append_data_test()
{
    QDataStream stream;
    stream.setDevice(file_);
    stream.setVersion(QDataStream::Qt_5_9);
...
void single_segd_files::open_data()
{
    if (!file_->open(QIODevice::Append))
    {
        qDebug() << file_->errorString();
...
void single_segd_files::close_data()
{
    file_->flush();
    file_->close();
}

void single_segd_files::openFile_test(QString fileReadName)
{
    fileRead_->setFileName(fileReadName);
    if (!fileRead_->open(QIODevice::ReadOnly))
    {
...
void single_segd_files::getHeaderData(QString fileReadName)
{
    fileRead_->setFileName(fileReadName);
    if (!fileRead_->open(QIODevice::ReadOnly))
    {
    ...
    /* ------------------Channel set Descriptor ----------------------------------------*/
    scan_type_header scn_type_hdr;
    qDebug() << "channel Sets" << channelSets_;
    for(int i=0; i <channelSets_; i++)
        streamData.readRawData((char*)&scn_type_hdr, sizeof (scn_type_hdr));
...
dem_trace_header single_segd_files::getDemuxTraceHeader()
{
    QDataStream streamData;
    streamData.setDevice(fileRead_);
    streamData.setVersion(QDataStream::Qt_5_9);
    ...
    return strct_read_mes;
}

/* -------------------Read Measure ---------------------------*/
QVector<float> single_segd_files::getDataRev3(uint32_t sizeData)
{
    QDataStream streamData;
    streamData.setDevice(fileRead_);
    streamData.setVersion(QDataStream::Qt_5_9);
...
/* Проверить на наличие данных в файле */
bool single_segd_files::checkData(QString fileCheckName)
{
    QFile fileCheck;
