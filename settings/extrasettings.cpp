    ...
    numWellsMod_(numWellsMod)
{
    ...
    for(int i = 0; i < numWellsMod_; i++)
        inversAndGainValue_.vecGainGroundModules.push_back(minIntGround);
...
void ExtraSettings::on_saveAndCloseButton_clicked()
{
    ...
    int gainGroundHole = ui->comboBox->currentText().toInt();
    if((gainDwnHole <= 0 || gainDwnHole >= 1000) || (gainGroundHole < -1000 || gainGroundHole > 1000))
    {
        QMessageBox* pmbx =
                            new QMessageBox("Ошибка",
